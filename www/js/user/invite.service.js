angular.module('starter').service('inviteService', function($rootScope, $q, $http, localStorage, ANS_WS_URI) {
	...
	function _status(email) {
		...
		return $http.get(ANS_WS_URI + "api/invites/status/" + email)
			.then(function(res) {
	...
	function _requestInvite(appUser) {
		...
		return $http.post(ANS_WS_URI + "api/invites/request", inviteRequest)
			.then(function(res) {
				return res.data;
			});
	}
