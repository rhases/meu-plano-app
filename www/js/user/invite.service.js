angular.module('starter').service('inviteService', function($rootScope, $q, $http, localStorage, SCHEDULER_HOST) {
	...
	function _status(email) {
		...
		return $http.get(SCHEDULER_HOST + "api/invites/status/" + email)
			.then(function(res) {
	...
	function _requestInvite(appUser) {
		...
		return $http.post(SCHEDULER_HOST + "api/invites/request", inviteRequest)
			.then(function(res) {
				return res.data;
			});
	}
